...
try {
  ...
  const pkg = join(out, `Lectern-Companion-${version}-macOS-${arch}${release ? '' : '-UNSIGNED-DEV'}.pkg`)
  ...
  console.log(`Built ${pkg}\nBound extension: ${id}\n${release ? 'Signed, notarized and stapled' : 'DEVELOPMENT ONLY: unsigned, not a Web Store release'}`)
