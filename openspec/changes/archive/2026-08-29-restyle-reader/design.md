...
### D7 深色截图的守卫:守终态,不守"没在动"(1c 二轮 review 后补)

第一版在深色截图前只等"过渡稳定"。那是个**负向条件**("没在动"),而
`waitStyleSettled` 用"连续两次读到同一个值"实现时,**起点静止同样满足它** ——
若在过渡还没开始时就来轮询,头两次读到的都是初始值,于是判"已稳定"直接放行。
一张浅色的图能就这么过闸,而断言是绿的。

改为两道闸,顺序不能反:
1. **终态守卫(正向)**:`data-theme === 'dark'` **且** `body` 实际背景 = 当前 `--bg` 令牌
   **且** 该值 ≠ 切换前的浅色底。守的是"深色真的画出来了",与过渡时序无关。
2. **过渡守卫**:再等 `.nav-btn` 背景稳定,且必须传 `from`(切换前的值)——
   先看见它**离开过**起始值,再等它停下来。

两条都写成 `check`(会红的断言),不是静默等待;`shot-7-dark` 与 `shot-14-dark-project`
各一份。已注入反证:把切主题那一步去掉后,终态守卫如实变红
(`theme=light`、底色未变),另一张的同名断言保持绿。
