...
两种视图复用同一个 `useResizable` 宽度状态和同一个拖拽入口语义：文件树与变更文件列表使用相同的像素宽度、最小/最大值和 `localStorage` 键。不能给 Git 视图维护第二个宽度值，否则切换时仍会发生几何跳变，两个视图也会逐渐漂移。

Git 视图在当前项目第一次打开后保持挂载，切回 `files` 只隐藏它，不销毁组件树。这样端点、模式、选中文件、已渲染 diff 及 CodeMirror 滚动位置由原对象直接保留，而不是复制出一套容易失真的恢复状态。只有项目根变化或离开 `project` 模式才卸载该会话。
...
worker 生命周期跟随“当前项目的已打开 Git 会话”，不跟随 `files | changes` 的可见性。第一次进入 `changes` 时才 lazy-load 并启动；临时切回 `files` 时组件和 worker 都保持存活，切回时不得重新发送 `probe-and-list-refs` 或 `compare`。项目根变化或离开项目模式时才执行 `dispose` 并终止 worker。

界面提供显式“重新比较”动作。它增加本地 compare generation，沿用当前端点和比较模式重新读取本地状态，并允许清空旧结果、显示加载态和选择新结果中的文件；它不等同于视图切换，也不隐式改变端点。
...
- [隐藏的 Git 视图会在文件阅读期间继续占用缓存] → 仅在当前项目首次打开 Git 后保留，worker 对象缓存仍受 32 MB LRU 上限约束；项目切换或离开项目模式立即释放。用这段有界内存换取切换视图时不重复扫描仓库、不丢失阅读现场。
