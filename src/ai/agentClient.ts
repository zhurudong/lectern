...
export type AgentMessage = { type: 'ready' } | { type: 'data'; data: string }
  ...
  | { type: 'waiting'; message: string; code?: string } | { type: 'project'; cwd: string }
...
export function connectAgent(settings: AgentSettings, project: AgentProject, cols: number, rows: number,
  ...
  const connect = () => {
    ...
    receive({ type: 'waiting', code: 'starting', message: '正在启动本机伴随程序…' })
    ...
    port.onMessage.addListener((message) => {
      ...
      if (message.type === 'hello') {
        ...
        send({ type: 'project', ...project, reselect, locale: document.documentElement.lang })
    ...
    port.onDisconnect.addListener(() => {
      ...
      receive({ type: 'waiting', code: 'reconnecting', message: '连接中断，正在重新启动本机程序；将开始新会话。' })
