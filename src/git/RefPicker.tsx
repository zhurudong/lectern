...
function errorText(error: unknown): string {
  ...
  return t('refpick.shaUnresolved')
}
...
export function RefPicker({ side, refs, client, selected, onSelect, onClose }: RefPickerProps) {
  ...
  const resolveSha = async () => {
    ...
    if (!/^[0-9a-f]{4,40}$/i.test(input)) {
      setShaState({ kind: 'error', message: t('refpick.shaInvalid') })
      return
    }
    setShaState({ kind: 'loading' })
    try {
      const payload = await client.resolveCommit(input)
      if (payload.type !== 'resolve-commit-result') throw new Error(t('refpick.shaUnknownResult'))
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {snapshot && <span class="git-snapshot-badge">{t('refpick.localSnapshot')}</span>}
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={t('refpick.select', { side: side === 'base' ? t('git.factBase') : t('git.factTarget') })}
      ref={panelRef}
      onKeyDown={onKeyDown}
    >
      <div class="git-ref-search-wrap">
        ...
        <input
          ...
          placeholder={t('refpick.filterPh')}
          aria-label={t('refpick.filterLabel')}
        />
        <kbd>Esc</kbd>
      </div>
      <div class="git-ref-options" role="listbox" aria-label={t('refpick.refsLabel')}>
        <section class="git-ref-group" aria-label={t('git.worktree')}>
          <div class="git-ref-group-title">{t('git.worktree')}<span>1</span></div>
          <button
            type="button"
            class="git-ref-option"
            role="option"
            ...
            disabled={side === 'base'}
            title={side === 'base' ? t('refpick.worktreeTargetOnly') : undefined}
            onClick={() => choose({ kind: 'worktree', label: t('git.worktree'), headOid: refs.head.oid })}
          >
            <span class="git-ref-option-name">{t('git.worktree')}</span>
            <span class="git-ref-option-note">{t('refpick.worktreeNote')}</span>
            {selected.kind === 'worktree' && <span aria-hidden="true">✓</span>}
          </button>
        </section>
        {renderGroup(t('refpick.localBranches'), groups.local)}
        {renderGroup(t('refpick.remoteBranches'), groups.remote, true)}
        {renderGroup('Tag', groups.tag)}
        {!query || groups.local.length + groups.remote.length + groups.tag.length > 0 ? null : (
          <div class="git-ref-no-result">{t('refpick.noMatch')}</div>
        )}
      </div>
      <form
        class="git-sha-entry"
      ...
      >
        <label for={`git-sha-${side}`}>{t('refpick.enterSha')}</label>
        <div>
          <input
            id={`git-sha-${side}`}
            value={sha}
            ...
            placeholder={t('refpick.shaPh')}
            spellcheck={false}
          />
          <button type="submit" disabled={!sha.trim() || shaState.kind === 'loading'}>
            {shaState.kind === 'loading' ? t('refpick.resolving') : t('refpick.use')}
          </button>
        </div>
