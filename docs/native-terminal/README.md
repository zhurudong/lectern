...
目录选择在系统文件选择器完成。终端工具栏的停靠按钮可在阅读区右侧与底部之间切换；底部停靠时，文件目录树保持全高。拖动分隔线调整宽度或高度，两种尺寸与停靠位置分别记住；分隔线也支持方向键调整。切换停靠位置、主题或界面语言会保留当前 CLI 会话与输出。

工具栏保留连接状态、重新连接、停靠、设置和关闭。CLI、完整目录、重新关联以及安装说明集中在设置中；首次出现的数据处理提示可收起，以后仍能在设置中查看。关闭结束 CLI，重新打开/断线重连是新会话。移动目录、清理浏览器数据或安装到不同扩展 ID 可能要求重新关联。
